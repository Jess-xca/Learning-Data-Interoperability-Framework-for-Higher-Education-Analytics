...
export default function Footer({
  ...
  if (variant === "minimal") {
    return (
      <footer
        className={`bg-surface-container-lowest/30 border-t border-outline-variant/10 py-8 ${className}`}
      >
            ...
            <div className="flex gap-8">
              {[
                ...
                <a
                  key={link.label}
                  href={link.href}
                  className="text-[9px] font-black text-on-surface-variant uppercase tracking-widest hover:text-primary transition-all opacity-40 hover:opacity-100"
                >
                  {link.label}
                </a>
              ))}
  ...
  return (
    <footer
      className={`bg-surface-container-lowest/40 border-t border-outline-variant/10 py-8 overflow-hidden ${className}`}
    >
      <div className="max-w-7xl mx-auto px-6 lg:px-12">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8 mb-6">
          ...
          <div className="space-y-3">
            <div>
              <span className="text-sm font-black text-on-surface tracking-tight block">
                Academic Curator
              </span>
          ...
          <div className="space-y-2">
            <h3 className="text-[9px] font-black text-primary uppercase tracking-widest opacity-60 mb-3">
              Resources
            </h3>
            <ul className="space-y-2">
              {["Documentation", "API Guide", "Support", "Status"].map(
                (link) => (
                  <li key={link}>
                    <a
                      href="#"
                      className="text-[11px] font-medium text-on-surface-variant hover:text-primary transition-all opacity-70 hover:opacity-100"
                    >
                      {link}
                    </a>
                  </li>
          ...
          <div className="flex gap-6">
            {["Privacy", "Terms", "Accessibility"].map((link) => (
              <a
                key={link}
                href="#"
                className="text-[10px] font-bold text-on-surface-variant uppercase tracking-widest opacity-50 hover:opacity-100 transition-all"
              >
                {link}
              </a>
            ))}
