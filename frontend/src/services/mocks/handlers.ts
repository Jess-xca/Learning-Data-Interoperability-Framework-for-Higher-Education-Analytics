...
// Mock data
const mockStudents = [
  { id: '202502SENG001', name: 'Jean Umucyo', email: 'jean@institution.edu', gpa: 3.92, program: 'Engineering', enrollmentYear: 2022, status: 'active' as const },
  { id: '202502SENG002', name: 'Claire Habimana', email: 'claire@institution.edu', gpa: 3.65, program: 'Engineering', enrollmentYear: 2022, status: 'active' as const },
  { id: '202502BUS001', name: 'David Mugisha', email: 'david@institution.edu', gpa: 3.45, program: 'Business', enrollmentYear: 2023, status: 'active' as const },
];

const mockCourses = [
  { id: 'SENG101', code: 'SENG101', name: 'Introduction to Programming', instructor: 'Dr. Kagame', credits: 3, enrollment: 45 },
  { id: 'SENG102', code: 'SENG102', name: 'Data Structures', instructor: 'Eng. Kubwimana', credits: 4, enrollment: 38 },
  { id: 'BUS101', code: 'BUS101', name: 'Business Management', instructor: 'Prof. Tsingyun', credits: 3, enrollment: 52 },
];
...
export const handlers = [
  ...
  http.get(`${API_BASE}/students/:id`, ({ params }) => {
    ...
    return HttpResponse.json({ success: false, error: 'Student not found' }, { status: 404 });
  }),

  http.post(`${API_BASE}/students`, async ({ request }) => {
    const newStudent = (await request.json()) as Record<string, unknown>;
    return HttpResponse.json({ success: true, data: { id: 'new-id', ...newStudent } }, { status: 201 });
  }),

  http.put(`${API_BASE}/students/:id`, async ({ request }) => {
    const updatedStudent = (await request.json()) as Record<string, unknown>;
    return HttpResponse.json({ success: true, data: { id: (request as any).params.id, ...updatedStudent } });
  }),
